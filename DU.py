...
def gettemp(ts,o_cfg):

    # NB *** one wire network has been enabled on BCM pin 17 (Pi pin  11) ***
    # Temp sensor type DS18B20
    # hid = HEX ID of the sensors
    if ts == "On_Board" :   
        hid = o_cfg.GetConfigValue("Temp_01")   
    
    elif  ts == "External": 
        hid = o_cfg.GetConfigValue("Temp_02")
        
    elif  ts == "Internal": 
        hid = o_cfg.GetConfigValue("Temp_03")
               
    tempfile = "/sys/bus/w1/devices/"+hid+"/w1_slave" 
